 ...
 * ft8_lib headers.
 * Compile with:
 *   -Ift8_lib/common -Ift8_lib/ft8 -Ift8_lib/fft -Ift8_lib/utils
 */
#include "constants.h"
#include "decode.h"
#include "message.h"
...
/*
 * Common FT8 USB dial frequencies.
 * These are dial frequencies, not exact tone frequencies.
 */
...
/*
 * Program global state, intentionally similar to dump1090's global Modes style.
 */
static struct {
...
static void sigintHandler(int sig) {
    ...
    /*
     * Do not call libiio cleanup functions from inside a signal handler.
     * The main loop notices Modes.stop and performs cleanup safely.
     */
}
...
/*
 * Wait for the next FT8 15-second UTC boundary.
 */
...
/*
 * Capture real-valued 12 kHz samples from Pluto IQ.
 *
 * FT8 over USB appears as audio tones above dial frequency.
 * Taking the I component of zero-IF IQ gives a real sinusoid that ft8_lib can process.
 *
 * This uses a simple boxcar decimator. It is intentionally small and cheap enough
 * to run on the Pluto. A future version should use a proper FIR decimator.
 */
static int captureAudioWindow(float *audio, int max_audio_samples) {
    ...
    while (!Modes.stop && out_count < max_audio_samples) {
        ...
        for (; p_dat < p_end && out_count < max_audio_samples; p_dat += p_inc) {
            ...
            /*
             * Scale signed 12/16-bit I samples into roughly -1..1.
             * AD9361 samples are commonly 12-bit left-justified in int16.
             */
